...
def nltk_summarizer(docx):
    stopWords = set(stopwords.words("english"))    //used to select all stopwords from doc and store them stopWords
    words = word_tokenize(docx)                    //used to tokenize the doc into word unit
    freqTable = dict()                             //creates a dictionary
    ...
    for word in words:
        ...
        if word not in stopWords:
            if word in freqTable:
                ...
                freqTable[word] = 1                //Its the count of every word which is not present in stopwords

    sentence_list= sent_tokenize(docx)
    #sentenceValue = dict()
    max_freq = max(freqTable.values())
    for word in freqTable.keys():
        freqTable[word] = (freqTable[word]/max_freq)    //priority is given to the sentence based on frequency of words present in it where key is word and value is its frequency
    ...
    summary = ' '.join(summary_sentences)                                     //TO get the sentence with Highest Priority
    return summary

#Function for SPACY                               //Same code as for NLTK
...
def main():                                                                  //This is for interface 
